...
for i in range(10):
    ...
    bit_flipped_weights = manipulate_single_bit(weights, 1)
    ...
    t1 = time.time()
    t_avg.append(t1 - t0)
    print("Time for Error Check: %f"%(t1-t0))
    print(i)

acc_avg = sum(acc_avg)/len(acc_avg)
loss_avg = sum(loss_avg)/len(loss_avg)
print("Avg Acc: %f\nAvg Loss: %f\nTime for Check: %f\nErrors Detected: %f"%(acc_avg, loss_avg, t_avg[0], errorsDetected))
# swap_attack_weights = manipulate_swap(weights)
#
# swap_attack_model = set_weights(model, swap_attack_weights, bias)
#
# score = swap_attack_model.evaluate(x_test, y_test, verbose=0)
# print("\nSWAP ATTACK - ")
# print("Test loss:", score[0])
# print("Test accuracy:", score[1])
